...
- **参数过滤**: 支持按类别（category）和输入模态（input_modalities）过滤模型
...
- 缓存键基于 category 和 input_modalities 参数生成，确保不同参数组合使用独立缓存
...
def _get_cache_key(category: str, input_modalities: str) -> str:
    """生成缓存键"""
    return f"openrouter:{category or 'all'}:{input_modalities or 'all'}"
...
def extract_free_models_from_api(category: str = None, input_modalities: str = None, cache_timeout: int = 300) -> List[str]:
    """
    ...
    """
    # 生成缓存键
    cache_key = _get_cache_key(category, input_modalities)
    ...
    try:
        ...
        # 提取所有模型的slug（这就是模型ID）
        model_ids = [model.get('slug') for model in models if model.get('slug')]
...
def get_models(plugin_config: Dict[str, Any]) -> List[str]:
    ...
    # 调用核心函数获取模型列表
    model_ids = extract_free_models_from_api(category, input_modalities, cache_timeout)
